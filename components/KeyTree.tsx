...
const NODE_W = 200;
const NODE_H = 66; // nominal height used for edge anchoring / centering
const COL_GAP = 258;
const ROW_GAP = 96;
const PAD = 48;
...
export default function KeyTree() {
  ...
  /* ---- Layout: assign each leaf a row, each parent the mean of its children ---- */
  const { pos, width, height } = useMemo(() => {
    ...
    const place = (id: string, col: number): number => {
      ...
      const ra = place(q.a.to, col + 1);
      const rb = place(q.b.to, col + 1);
      const row = (ra + rb) / 2;
      pos[id] = { col, row };
      return row;
    };
    place(rootId, 0);
    const cols = Math.max(...Object.values(pos).map((p) => p.col));
    const width = PAD * 2 + cols * COL_GAP + NODE_W;
    const height = PAD * 2 + (leaf - 1) * ROW_GAP + NODE_H;
    return { pos, width, height };
  }, []);

  /* ---- Parent map + path-to-root for any node ---- */
  ...
  const left = (id: string) => PAD + pos[id].col * COL_GAP;
  const top = (id: string) => PAD + pos[id].row * ROW_GAP;
  ...
  const edges = useMemo(() => {
    ...
    Object.entries(questions).forEach(([id, q]) => {
      list.push({ from: id, to: q.a.to });
      list.push({ from: id, to: q.b.to });
    });
    return list;
  }, []);
  ...
  const selectChoice = (qid: string, key: "a" | "b") => {
    setPath([...pathTo(qid), questions[qid][key].to]);
  };

  const nodeLabel = (id: string) =>
    isSpecies(id) ? species[id].common : `Q${questions[id].num}`;
  const choiceShort = (parent: string, child: string) =>
    questions[parent].a.to === child
      ? questions[parent].a.short
      : questions[parent].b.short;

  const allIds = [...Object.keys(questions), ...Object.keys(species)];
  ...
  return (
    <div>
        ...
        <div className="flex flex-wrap items-center gap-x-1.5 gap-y-2 text-sm">
          ...
          {isSpecies(frontier) && (
            <span className="ml-1 rounded bg-copper/20 px-2 py-1 text-xs font-semibold text-copper-soft">
              ✔ identified: <i>{species[frontier].binomial}</i>
            </span>
          )}
        </div>
      </div>
      ...
      <div
        className="tree-scroll overflow-auto rounded-lg border border-teal-line/60 bg-ink-2/30"
      ...
      >
        <div className="relative" style={{ width, height }}>
            ...
            {edges.map((e) => {
              ...
              return (
                <path
                  ...
                  style={
                    lit
                      ? { filter: "drop-shadow(0 0 4px rgba(198,123,71,.7))" }
                      : undefined
                  }
                />
              );
          ...
          {allIds.map((id) => {
            ...
            const leaf = isSpecies(id);
            const region = leaf
              ? kingdomMeta[species[id].kingdom]
              : regionMeta[questions[id].region];
              ...
              >
                <div
                  className={`card-parchment overflow-hidden transition-shadow ${
                    active
                      ? "shadow-glow ring-2 ring-copper"
                      : inPath
                        ? "ring-2 ring-copper/70"
                        : "ring-1 ring-black/10"
                  }`}
                >
                  {/* region / kingdom accent bar */}
                  <div style={{ height: 4, background: region.color }} />

                  {leaf ? (
                    /* ---------- Species endpoint ---------- */
                    <button
                      onClick={() => setPath(pathTo(id))}
                    ...
                    >
                      ...
                      <span className="mt-0.5 block text-xs italic text-copper-deep">
                        {species[id].binomial}
                      </span>

                      {active && (
                        <div className="mt-2 border-t border-copper-deep/25 pt-2">
                          <p className="text-[0.68rem] font-medium text-bodyink/70">
                            {species[id].group}
                          </p>
                          <ul className="mt-1.5 space-y-1">
                            {species[id].traits.slice(0, 3).map((t, i) => (
                          ...
                          <ImagePlaceholder
                            species={species[id]}
                            className="mt-2 h-16 text-[0.6rem]"
                          />
                        </div>
                      )}
                    </button>
                    ...
                    <div className="p-3">
                      ...
                      >
                        <span className="flex items-center gap-2">
                          <span
                            className="flex h-5 min-w-[1.4rem] items-center justify-center rounded px-1 font-display text-[0.7rem] font-bold text-white"
                            style={{ background: region.color }}
                          >
                            {questions[id].num}
                          </span>
                          <span className="text-xs font-semibold leading-tight text-bodyink">
                            {questions[id].short}
                          </span>
                        </span>
                      </button>
                      ...
                      {active && (
                        <Rich
                          as="p"
                          html={questions[id].question}
                          className="card-parchment mt-2 block text-[0.72rem] leading-snug text-bodyink/85"
                        />
                      )}
                      ...
                      <div className="mt-2 flex flex-col gap-1">
                        {(["a", "b"] as const).map((k) => {
                          const isNext =
                            frontier === id ||
                            litPairs.has(`${id}>${questions[id][k].to}`);
                          return (
                            <button
                              key={k}
                              onClick={(e) => {
                            ...
                            >
                              ...
                              {active ? (
                                <Rich html={questions[id][k].label} />
                              ) : (
                                <span>{questions[id][k].short}</span>
                              )}
                            </button>
                          );
                        })}
